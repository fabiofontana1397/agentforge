    ...
    <DashboardLayout>
      <div style={{ maxWidth: 1100 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 32 }}>
          <div>
            <h1 style={{ fontFamily: '"Playfair Display", serif', fontSize: 32, fontWeight: 900, letterSpacing: '-1px', marginBottom: 4 }}>I miei Agenti</h1>
            <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 15 }}>Crea e gestisci i tuoi assistenti AI</p>
          </div>
          <button onClick={() => { setCreating(true); setSelectedAgent(null) }} style={{ background: 'linear-gradient(135deg, #00D4AA, #00A3FF)', color: '#000', border: 'none', padding: '12px 24px', borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: '"DM Sans", sans-serif', boxShadow: '0 0 20px rgba(0,212,170,0.2)' }}>
            + Nuovo agente
          </button>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: selectedAgent || creating ? '1fr 1fr' : '1fr', gap: 24 }}>
          ...
          <div>
            {loading ? (
              ...
              <div style={{ background: 'rgba(255,255,255,0.02)', border: '2px dashed rgba(255,255,255,0.08)', borderRadius: 16, padding: '60px', textAlign: 'center' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {agents.map(agent => (
                  <div key={agent.id} onClick={() => { setSelectedAgent(agent); setCreating(false) }} style={{ background: selectedAgent?.id === agent.id ? 'rgba(0,212,170,0.08)' : 'rgba(255,255,255,0.03)', border: selectedAgent?.id === agent.id ? '1px solid rgba(0,212,170,0.35)' : '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: '20px 24px', cursor: 'pointer', transition: 'all 0.2s' }}
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                        <div style={{ width: 36, height: 36, borderRadius: 8, background: 'linear-gradient(135deg, rgba(0,212,170,0.2), rgba(0,163,255,0.2))', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>🤖</div>
                        <div>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        <button onClick={e => { e.stopPropagation(); navigate(`/chat/${agent.id}`) }} style={{ background: 'rgba(0,212,170,0.1)', border: '1px solid rgba(0,212,170,0.2)', color: '#00D4AA', padding: '6px 14px', borderRadius: 6, fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: '"DM Sans", sans-serif' }}>
                          Chat →
                        </button>
                      </div>
                    </div>
                    <p style={{ color: 'rgba(255,255,255,0.3)', fontSize: 13, lineHeight: 1.5, display: '-webkit-box', WebkitLineClamp: 1, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{agent.system_prompt}</p>
                  </div>
                ))}
              </div>
            )}
          ...
          {/* PANNELLO DESTRA: CREA O DETTAGLIO */}
          {creating && (
            <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(0,212,170,0.2)', borderRadius: 16, padding: '28px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
                <h2 style={{ fontFamily: '"Playfair Display", serif', fontSize: 22, fontWeight: 700 }}>Nuovo agente</h2>
                <button onClick={() => setCreating(false)} style={{ background: 'transparent', border: 'none', color: 'rgba(255,255,255,0.3)', fontSize: 20, cursor: 'pointer' }}>✕</button>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div>
                  ...
                  <input value={name} onChange={e => setName(e.target.value)} placeholder="Es. NutriCoach AI" style={{ width: '100%', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, padding: '12px 14px', color: '#fff', fontSize: 14, fontFamily: '"DM Sans", sans-serif', outline: 'none' }}
                    onFocus={e => e.target.style.borderColor = 'rgba(0,212,170,0.5)'}
                    onBlur={e => e.target.style.borderColor = 'rgba(255,255,255,0.1)'}
                  />
                </div>
                <div>
                  <label style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', fontWeight: 600, letterSpacing: '1px', textTransform: 'uppercase', display: 'block', marginBottom: 8 }}>Istruzioni (system prompt)</label>
                  <textarea value={prompt} onChange={e => setPrompt(e.target.value)} placeholder="Es. Sei un assistente nutrizionale esperto. Rispondi sempre in italiano, sii empatico e professionale..." rows={6} style={{ width: '100%', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, padding: '12px 14px', color: '#fff', fontSize: 14, fontFamily: '"DM Sans", sans-serif', outline: 'none', resize: 'vertical' }}
                    onFocus={e => e.target.style.borderColor = 'rgba(0,212,170,0.5)'}
                    onBlur={e => e.target.style.borderColor = 'rgba(255,255,255,0.1)'}
                  />
                </div>
                <button onClick={createAgent} disabled={saving || !name.trim() || !prompt.trim()} style={{ background: 'linear-gradient(135deg, #00D4AA, #00A3FF)', color: '#000', border: 'none', padding: '14px', borderRadius: 8, fontSize: 15, fontWeight: 700, cursor: 'pointer', fontFamily: '"DM Sans", sans-serif', opacity: saving || !name.trim() || !prompt.trim() ? 0.5 : 1, transition: 'opacity 0.2s' }}>
          ...
          {/* DETTAGLIO AGENTE SELEZIONATO */}
          {selectedAgent && !creating && (
            <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(0,212,170,0.2)', borderRadius: 16, padding: '28px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
                <div>
                  <h2 style={{ fontFamily: '"Playfair Display", serif', fontSize: 22, fontWeight: 700, marginBottom: 4 }}>{selectedAgent.name}</h2>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                  ...
                  <label style={{ background: 'rgba(0,212,170,0.1)', border: '1px solid rgba(0,212,170,0.2)', color: '#00D4AA', padding: '6px 14px', borderRadius: 6, fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: '"DM Sans", sans-serif' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {selectedAgent.documents.map(doc => (
                      <div key={doc.id} style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 8, padding: '10px 14px' }}>
                        ...
                        <button onClick={() => deleteDocument(doc.id)} style={{ background: 'transparent', border: 'none', color: 'rgba(255,100,100,0.5)', cursor: 'pointer', fontSize: 16, padding: '0 4px' }}
                          onMouseEnter={e => e.target.style.color = '#ff6b6b'}
                          onMouseLeave={e => e.target.style.color = 'rgba(255,100,100,0.5)'}
                        >🗑️</button>
                      </div>
                    ))}
                  </div>
